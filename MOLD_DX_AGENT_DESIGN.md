...
> 작성일: 2026-06-11
> 목적: 금형 사출 데이터(금형DX 과제)를 입력받아 **기술분석서**(분석 범위 / SW 개발 방향 / 라이브러리 / 프로토타입 가능성·효과)를 자동 작성하는 에이전트의 아키텍처를 설계한다.
...
- **기존 책 생성 에이전트는 손대지 않는다.** 이번에 만드는 것은 **기술서 전용의 독립 에이전트**다. (장기적으로는 책·기술서·소설처럼 *문서 유형별 전용 에이전트*가 나란히 존재하는 구조 — §3.0 포지셔닝)
- 다만 책 에이전트가 검증한 **패턴(Writer→Review→Revise 루프, 단계별 temperature, 품질 로그)은 차용**한다. 코드를 공유할지(공용 코어 추출), 복붙으로 시작할지는 §8에서 선택.
- 핵심 신규 컴포넌트는 **Data Digest Adapter(데이터 다이제스트 어댑터)** 하나다. 517,130 사이클의 원본을 LLM에 넣지 않고, **결정론적 코드가 통계 요약(digest)을 만들어** 프롬프트에 주입한다. 이게 "환각 없는 기술분석서"의 핵심이다.
...
- **장기 프롬프트 체인 수렴 문제**는 단계별 출력을 **Pydantic 스키마로 강제**해서 해결한다 (현재의 정규식 JSON 파싱 대체). §6.
...
| R5 | **기존 책 에이전트는 무수정** — 기술서 엔진은 독립 | 책 코드 diff 0 |
| R6 | 컨텍스트 초과 방지 | 원본 미주입, digest만 주입 |

---

## 3.0 포지셔닝 — "장르별 전용 에이전트" 군 (중요)

이번 작업은 기존 에이전트의 개조가 아니라 **새 식구를 들이는 일**이다. 큰 그림은 *문서 유형(장르)마다 전용 에이전트가 따로 존재*하는 구조다.

```mermaid
flowchart LR
    subgraph CORE["공용 코어 (선택적 공유 — §8)"]
        LOOP["생성 루프\nWriter→Review→Revise"]
        STRUCT["call_structured()\nPydantic 수렴 엔진"]
        STORE["저장/로그/푸시"]
    end
    subgraph AGENTS["장르별 전용 에이전트 (각자 독립)"]
        BOOK["📘 책 에이전트\n(현행, 무수정)\nTOC → 챕터"]
        TECH["📊 기술서 에이전트\n(이번 신규)\nDataDigest → 분석 섹션"]
        NOVEL["📖 소설 에이전트\n(향후)\n설정/플롯 → 장면"]
    end
    LOOP & STRUCT & STORE -.재사용.-> BOOK
    LOOP & STRUCT & STORE -.재사용.-> TECH
    LOOP & STRUCT & STORE -.재사용.-> NOVEL
```

각 에이전트가 다른 점은 **세 가지뿐**이고, 나머지(루프·검수·저장)는 공유 가능하다.

| 장르 | ① 입력 사양 | ② 그라운딩 | ③ 시스템 프롬프트(문체/구성) |
| --- | --- | --- | --- |
| 책(현행) | TOC 챕터 | 없음(모델 지식) | 교재 문체 |
| **기술서(이번)** | report_spec 섹션 | **DataDigest(실측)** | 기술문서 문체 |
| 소설(향후) | 플롯/캐릭터 시트 | 세계관 바이블 | 서사 문체 |

> 즉 "기술서 전용"이란 **이 3요소를 기술서에 맞게 새로 정의한 독립 패키지**를 만든다는 뜻이다. 책 에이전트 코드는 그대로 둔다(R5). 소설도 같은 틀로 나중에 추가하면 된다.
...
flowchart TD
    ...
    subgraph PIPE["③ 에이전트 파이프라인 (기존 재사용)"]
        PLAN["Planner\n섹션별 분석 계획"]
        WRITE["Writer t=0.7\n섹션 초안"]
...
1. **Data Digest Adapter (신규)** — 데이터 소스를 추상화하고, LLM이 먹을 수 있는 **압축된 검증 객체(DataDigest)** 를 만든다.
2. **분석서 사양(report_spec)** — 책의 TOC에 해당. 4개 필수 섹션을 정의한다.
3. **에이전트 파이프라인 (기존)** — `book_writer.py`의 Writer/Review/Revise 루프를 거의 그대로 쓴다. 챕터 → 섹션으로 바뀔 뿐.
