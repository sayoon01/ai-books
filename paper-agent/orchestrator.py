...
from core.config import (OUTPUT_ROOT, MODEL, REVIEW_MODEL,
                         QUALITY_GATE, TARGET_SCORE, MIN_CHARS, WRITE_MAX, PASS_MAX)
...
def _write_with_guard(plan, section, grounding, prev, sec_arts, min_chars, log):
...
def run_paper(spec: dict, *, force: bool = False, limit: int = 0) -> dict:
    ...
    plan = run_or_load_plan(spec["topic"], source_text, out,
                            venue=spec.get("venue", ""), force=force)
    ...
    for i, section in enumerate(todo, 1):
        sid = section["id"]
        log(f"\n[{i}/{len(todo)}] 섹션 '{sid}' — {section.get('title','')}")
        sec_arts = [art_by_id[a] for a in section.get("artifact_ids", []) if a in art_by_id]
        min_chars = 280 if sid == "abstract" else MIN_CHARS

        draft, _ = _write_with_guard(plan, section, data_digest, prev_summaries,
                                     sec_arts, min_chars, log)

        state = {
            "plan": plan, "section": section, "draft": draft, "grounding": data_digest,
            "quality_gate": QUALITY_GATE, "target_score": TARGET_SCORE,
    ...
    meta = {"slug": slug, "model": MODEL, "review_model": REVIEW_MODEL,
            "sections": sec_meta,
